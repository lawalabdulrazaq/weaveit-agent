...
export default function Stats() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // Use the dedicated global stats endpoint for homepage stats
        const global = await backend.fetchGlobalStats()
        console.debug("Fetched global stats:", global)
        if (!mounted) return
        setPlatformStats({
  ...
  const stats = [
    { 
      number: platformStats?.count !== undefined ? formatNumber(platformStats.count) : "10K+", 
